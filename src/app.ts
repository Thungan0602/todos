import {createTodo, deleteTodo, fetchTodos, Todo, toggleTodo} from "./entities/todo";
...
function createTodoItem({ text, completed, id }: TodoItem) {
...
const renderApp = async () => {
  ...
  list.replaceChildren(...filteredTodos.map((todo) => createTodoItem(todo as TodoItem)));
...
list.onclick = async (e) => {
  ...
  if (target.getAttribute("data-todo") === "delete") {
    ...
    await deleteTodo(id as string);  // xóa trên database
  ...
  if (target.getAttribute("data-todo") === "toggle") {
    ...
    await toggleTodo(id as string, !todo!.completed);
...
clearCompleted.onclick = async() => {
  await Promise.all( // xóa tất cả cùng 1 lúc
    todos.filter((todo) => todo.completed).map((todo) => deleteTodo(todo.id as string)),
  );
